...
import { searchArtists, parseArtistId, spotifyConfigured, SpotifyError } from "@/lib/spotify";
...
export default async function FindPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  // A pasted artist link/ID jumps straight to the audit.
  ...
  if (configured && query) {
    try {
      ...
      error = e instanceof SpotifyError ? e.message : "Search failed.";
    }
  }
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 pb-24 pt-8">
      ...
      <p className="mt-1 text-sm text-neutral-500">Search an artist on Spotify, or paste their Spotify link.</p>
        ...
        <>
          ...
          <div className="mt-6 flex flex-col gap-2">
            {hits.map((a) => (
              ...
              >
                <span className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-white/5 ring-1 ring-white/10">
                  {a.image && (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 truncate text-[13px] text-neutral-500">
                    {listeners(a.followers)} followers{a.genres[0] ? ` · ${a.genres[0]}` : ""}
                  </div>
                </div>
